...
public class AutoCollector {
    ...
    private final double collectorExtensionSpeed = 0.20;
    ...
    //number of minibackups we've done this extension so far
    private int miniBackUps = 0;
...
//                myRobot.stopMovement();//we need to stop movement if we are done driving
                ...
                driveDirectlyToCollectingLocation();
//                driveDirectlyToCollectingLocation();
...
//                MovementVars.movement_y = 0.1;//move slowly in
                ...
                if(currTime - doneDrivingTime > 200){
                    ...
                    double perpAngle = (Math.abs(myCollector.deltaAngleFromPerpendicular) / Math.toRadians(30));
                    //yeah vroom the collector extension. Eat gobbies eat!
                    myCollector.setExtensionPowerRaw(collectorExtensionSpeed * 0.7
                                + (Math.abs(perpAngle)/Math.toRadians(30) * 0.13));//faster the more our angle is off


                            //Range.clip(((currTime-doneDrivingTime)-200)/600.0,0,1));//add acceleration
                }else{
                    myCollector.setExtensionPowerRaw(0);
                }
            ...
            if(collectorDeployed){
                ...
                myCollector.setAverageSpeed(0.6);
                nextStage();
            }
        }
        ...
        if(myState == myStates.putDumperDown.ordinal()
            ...
            myCollector.MeasureCurrentIfNecessary();

            driveDirectlyToCollectingLocation();
//            MovementVars.movement_y = 0.07;

            //update the collector's average speed
            myCollector.calcAverageSpeed();
            double currentCollectorAverageSpeed = myCollector.getExtensionCurrentSpeedPercent();//myCollector.getAverageSpeed();

            if(myCollector.getCurrMotorCurrentDraw() > 2700
                    && myCollector.getExtensionCurrSpeed() > -0.05
                    && myCollector.getExtensionPercent() > 0.1
                    && currTime - stateStartTime > 380){
        ...
        if(myState == myStates.putDumperDown.ordinal()){
            ...
            boolean exit = currTime - stateStartTime > 500;//1000
...
//            myCollector.setRunToPositionMode(Extension.RunToPositionModes.powerControl);
            ...
            if(collectorPredictedPos > COLLECTOR_GIVE_UP_PERCENT || currTime - stateStartTime > 4000){
                nextStage(myStates.turnAndRetractToNext.ordinal());
                //now we can remove the 0th index so we go to the next one
                nextCollectingLocation();
            }
        ...
        if(myState == myStates.miniBackUp.ordinal()){
            ...
            //back up lightly
            myCollector.setExtensionPowerNice(-0.85);

            //if we have traveled enough, go back to scouting for food
            if(initialCollectorMiniBackUpPos - myCollector.getExtensionPercent() > 0.07 ||
                    myCollector.getExtensionPercent() < 0.13){
                nextStage(myStates.scoutingForFood.ordinal());
            }
        }
        ...
        if(myState == myStates.delayBeforeScouting.ordinal()){
            ...
            if(currTime - delayCollectorDeployTime > 100 &&
                    delayCollectorDeployTime != 0){
                //go back to scouting for food
                nextStage(myStates.scoutingForFood.ordinal());
                myCollector.setAverageSpeed(0.6);//set this to a high value
            }
        }
    ...
    /**
     * Uses no pure pursuit and just drives right to the collecting location
     */
    private void driveDirectlyToCollectingLocation() {
        double savedTurn = movement_turn;
        ...
        double additionForAngle = (Math.abs(deltaAngleFromPerpendicular) / Math.toRadians(30)) * 15;

        double x = currLocation().xApproach + 20;
        double y = currLocation().yApproach + 20;
        x -= additionForAngle * Math.cos(worldAngle_rad);
        y -= additionForAngle * Math.sin(worldAngle_rad);

        //just go to the position, don't worry about point angle and stuff
        MovementEssentials.gunToPosition(x,y,-1,0.25,
                -1,Math.toRadians(30),0,false);
        movement_turn = savedTurn;
