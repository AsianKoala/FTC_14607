...
@Disabled
@Autonomous(name = "REAL Red Single Skystone", group = "Firefly")
public class RedSingleSkystoneEncoder extends LinearOpMode {
    ...
    private static float[] midPos = {4.2f/8f, 2.2f/8f};//0 = col, 1 = row
    private static float[] leftPos = {2.5f/8f, 2.2f/8f}; //2.7f/8f
    private static float[] rightPos = {5.9f/8f, 2.2f/8f}; // 3.4
//
...
//        double heading = getHeadingRaw180(startHeading);
    ...
    public void foundStone(int position){
        ...
        driveEncodersForwardIMU(680, 680, 680, 680, -.4, -.4, -.4, -.4, 0.25, 5, 0, false);
        ...
        if(position == 1) {
            driveEncodersStrafeIMU(3725, 3725, -3725, -3725, 0.8, 0.8, -0.8, -0.8, 0.5, 8, 0, true);
        } else if(position == 2) {
            driveEncodersStrafeIMU(3400, 3400, -3400, -3400, 0.8, 0.8, -0.8, -0.8, 0.5, 8, 0, true);
        } else if(position == 3) {
            driveEncodersStrafeIMU(3125, 3125, -3125, -3125, 0.8, 0.8, -0.8, -0.8, 0.5, 8, 0, true);
        }
...
//        }
    ...
    public void rotateToSquareResetLift(double targetHeading, double power, double timeoutSec){
        ...
        while(opModeIsActive() && currentHeading > targetHeading && (System.currentTimeMillis()-startTime)/1000 < timeoutSec) {
            ...
            if((System.currentTimeMillis()-startTime) > 1200) { // 1000
                rotater.setPosition(GLOBALS.rotaterHome);
            }
