...
@Autonomous(name = "REAL Red Odo Auto Skystone", group = "Firefly")
public class RedFinalOdoAutoSkystone2 extends LinearOpMode {
    ...
    private static float[] midPos = {2.9f/8f, 5f/8f};//0 = col, 1 = row
    private static float[] leftPos = {1.2f/8f, 5f/8f}; //2.7f/8f
    private static float[] rightPos = {4.6f/8f, 5f/8f};
//
...
//    private static float[] rightPos = {6.3f/8f, 2.0f/8f};
    ...
    public void runOpMode() throws InterruptedException {
        ...
        robot.clawRotater.setPosition(GLOBALS.frontClawRotaterHome);
...
//        ourSkystonePosition = GLOBALS.SKYSTONE_POSITION.MIDDLE;
        ...
        while(opModeIsActive()) {
            ...
            if(ourSkystonePosition == GLOBALS.SKYSTONE_POSITION.LEFT) {
                if(state == 1) {
                    ...
                    if(powers.get("done") != 1.0) {
                        ...
                        if(xOdoVal() > 10) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterOut);
                        }
                ...
                if(state == 8) {
                    powers = moveToPositionAbsolute(24, -75, 1.5, stateStartTime); // 0.75 thresh
                    if(powers.get("done") != 1.0) {
                        if(yOdoVal() < -5) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterBack);
                        }
                        if(yOdoVal() < -20) {
                ...
                if(state == 10) {
                    powers = moveToPositionAbsolute(24-1, 32, 0.25, stateStartTime); // thresh 0.25
                    if(powers.get("done") != 1.0) {
                        if(yOdoVal() > -72) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterBack);
                        }
                ...
                if(state == 14) {
                    powers = moveToPositionAbsolute(24, -80, 1.5, stateStartTime); // y 75 85 too far thresh 0.75
                    if(powers.get("done") != 1.0) {
                        if(yOdoVal() < -5) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterBack);
                        }
                ...
                if(state == 21) {
                    angleCorrectFront = false;
                    powers = rotateAbsolute(2, 5, stateStartTime, 3000);
                    powers.put("fr", -0.1);
                    powers.put("br", -0.1);
                    if(powers.get("done") != 1.0) {
            ...
            if(ourSkystonePosition == GLOBALS.SKYSTONE_POSITION.MIDDLE) {
                ...
                if(state == 10) {
                    powers = moveToPositionAbsolute(24-1, 24, 0.25, stateStartTime); // thresh 0.25
                    if(powers.get("done") != 1.0) {
                        if(yOdoVal() > -72) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterBack);
                        }
            ...
            if(ourSkystonePosition == GLOBALS.SKYSTONE_POSITION.RIGHT) {
                ...
                if(state == 10) {
                    powers = moveToPositionAbsolute(24-1, 16, 0.25, stateStartTime); // thresh 0.25
                    if(powers.get("done") != 1.0) {
                        if(yOdoVal() > -72) {
                            robot.clawRotater.setPosition(GLOBALS.frontClawRotaterBack);
                        }
