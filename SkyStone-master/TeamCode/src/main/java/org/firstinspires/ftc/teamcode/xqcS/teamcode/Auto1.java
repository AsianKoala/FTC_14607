package org.firstinspires.ftc.teamcode.xqcS.teamcode;
...
import static org.firstinspires.ftc.teamcode.xqcS.RobotUtilities.MovementEssentials.followCurve;
...
//        DrawVirtualField();
    ...
    public void start(){
         ...
         */
        double middleCubePercent = 0.435;
        double leftCubePercent = 0.45 - 0.23;
        double rightCubePercent = 0.45 + 0.19;
        ...
        if(cubeLocation == 2){
            for(int i = 0; i < 60-2; i += 3){
                ...
                myAutoCollector.allCollectingLocations.get(i+1)
                        .setPercentApproach(rightCubePercent + 0.035,homeCrater);
        ...
        if(cubeLocation == 1){
            for(int i = 0; i < 60-2; i += 3){
                ...
                myAutoCollector.allCollectingLocations.get(i+1).pointLocationY = 45;

                myAutoCollector.allCollectingLocations.get(i+2).pointLocationX = 45;
...
//        MyPosition.setPosition(147.67, 140.573, Math.toRadians(-45-0.55));
        ...
        programStage = progStates.landing.ordinal();//start from hang/normal
...
//                    0,0.01,0,false);
            ...
            if(Math.abs(r.turnDelta_rad) < Math.toRadians(6)){
                ...
                myCollector.setExtensionPowerRaw(0.5);
            ...
            double distToTarget = Math.hypot(getXPos()-ballX,getYPos()-ballY);

            if(myCollector.getCurrExtensionDistFromCenter() > distToTarget - 2){
                stopMovement();
                nextStage();
                myCollector.setExtensionPowerRaw(0);
            }
