...
public class Lift extends Extension{
    ...
    public static double offsetDumperServo = 0.045;
    public double DUMP_SERVO_ACTIVATED = 0.72 + offsetDumperServo;
    public double DUMP_SERVO_GOING_UP = 0.32 + offsetDumperServo;
    /** Starts in 18 inches like a good little robot */
    public double DUMPER_SERVO_18INCHES = 0.38 + offsetDumperServo;

    /** Where we go when at the bottom of our range */
    public double DUMP_SERVO_ZERO = 0.142 + offsetDumperServo;
...
//        myRobot.telemetry.addLine("differential: " + differential);
    ...
    private void updateMoveUpLift() {
        ...
        double predictedLiftPosition = getExtensionPercent() +
                getExtensionCurrentSpeedPercent()
                        * (SLIP_PERCENT_PER_SPEED_PERCENT * 0.40);
        ...
        super.setExtensionPowerRaw(Range.clip(delta / 0.22,minVal,1)
                * positionModePower);
    }
    ...
    private void updateResetLift(){
        ...
        if(Math.abs(getExtensionCurrentSpeedPercent()) < 0.2 &&
            ...
            liftErrorSum += delta * (elapsedTimeThisUpdate/125.0) * 1.5;
        }else{
            liftErrorSum = 0;
        }
        delta += liftErrorSum;
...
//        delta *= Range.clip(Math.abs(getExtensionPercent())/0.15,0.2,1);
        ...
        super.setExtensionPowerRaw(Range.clip(delta / 0.3,min,max) * positionModePower);
