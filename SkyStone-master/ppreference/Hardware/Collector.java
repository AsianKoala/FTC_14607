package org.firstinspires.ftc.teamcode.plswork.java.Hardware;
...
public class Collector extends Extension{
    ...
    /**
     * How much current we are currently drawing from the extension motor
     */
    private double currCurrentConsumption = 0;
...
//        }
    ...
    //the last time we measured the motor current draw
    private long lastMeasureCurrentDrawTime = 0;
    /**
     * Measures how much current we are consuming
     */
    public void MeasureCurrentIfNecessary() {
        long currTime = SystemClock.uptimeMillis();
        if(Auto1.programStage == Auto1.progStates.goToCollectMore.ordinal()
                && currTime - lastMeasureCurrentDrawTime > 40){
            lastMeasureCurrentDrawTime = currTime;
            currCurrentConsumption = extensionMotor.myMotor.getCurrentDraw();
        }
    }

    /**
     * Gets how much current we are drawing with the extension motor
     * @return a double
     */
    public double getCurrMotorCurrentDraw(){
        return currCurrentConsumption;
    }
...
//            myRobot.telemetry.addLine("CollectorDumperPercent: " + getTiltPercent());
     ...
     */
    private static double TIME_TO_DEPLOY_COLLECTOR = 0.04;


    /**
     * The angle we make with the line perpendicular to the crater
     */
    public double deltaAngleFromPerpendicular = 0;
    ...
    public boolean deployWhenOverCrater() {
        ...
        double distanceFromRobotToDeploy = getDistanceFromRobotUntilDeploy();
...
//                new FloatPoint(intercept.x - (Math.cos(myRobot.getAngle_rad()) * distanceUntilDeployWithSpeed),
//                            intercept.y - (Math.sin(myRobot.getAngle_rad()) * distanceUntilDeployWithSpeed));

//        ComputerDebugging.sendLine(intercept,secondPoint);
        ...
        double targetDeployAddition = 0;//10;//positive makes it deploy earlier, negative is later


        if(Auto1.cubeLocation == 0){
            targetDeployAddition += 6;//be more aggressive
        }
        if(Auto1.cubeLocation == 1){
            targetDeployAddition += 4;//be more aggressive
        }


//        deltaAngleFromPerpendicular = MyPosition.subtractAngles(myRobot.getAngle_rad(),Math.atan(m1)-Math.toRadians(90));
//        double additionForAngle = (Math.abs(deltaAngleFromPerpendicular) / Math.toRadians(30)) * 20;
...
//                additionForAngle + "\n\n");
    ...
    /**
     * gets the distance until where we should deploy the collector
     * @return
     */
    public double getDistanceFromRobotUntilDeploy() {
        /*
        -------------------------
        |                       |
        |                       |
        ...
        double slopeOfRobot = Math.tan(myRobot.getAngle_rad());

        double yLeft = myRobot.getYPos() + Math.sin(worldAngle_rad + Math.toRadians(90)) * 7.747;
        double xLeft = myRobot.getXPos() + Math.cos(worldAngle_rad + Math.toRadians(90)) * 7.747;
        double yRight = myRobot.getYPos() - Math.sin(worldAngle_rad + Math.toRadians(90)) * 7.747;
        double xRight = myRobot.getXPos() - Math.cos(worldAngle_rad + Math.toRadians(90)) * 7.747;



        /*Take the intercept of the two points */
        FloatPoint interceptLeft = MyMath.lineIntersecion(
                new FloatPoint(x1,y1),m1,new FloatPoint(xLeft,yLeft),slopeOfRobot);

        FloatPoint interceptRight = MyMath.lineIntersecion(
                new FloatPoint(x1,y1),m1,new FloatPoint(xRight,yRight),slopeOfRobot);

        //send the intersection for debugging
        ComputerDebugging.sendKeyPoint(interceptLeft);
        ComputerDebugging.sendKeyPoint(interceptRight);



        //get the distance we are away from the intercept
        double distanceToLeft = Math.hypot(interceptLeft.x-myRobot.getXPos(),
                interceptLeft.y-myRobot.getYPos());
        double distanceToRight = Math.hypot(interceptRight.x-myRobot.getXPos(),
                interceptRight.y-myRobot.getYPos());

        //return the maximum
        return distanceToLeft > distanceToRight ? distanceToLeft : distanceToRight;
    }
    ...
    public void setExtensionPowerNice(double power){
        ...
        if(predictedPosition > 1 && getExtensionCurrSpeed() > 0.2 && getExtensionPercent() < 1){
            power -= overShootEndRangePrediction * 0.9;
        }
