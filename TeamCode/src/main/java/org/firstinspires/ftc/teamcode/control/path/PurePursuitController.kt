...
//    private val mins = Pose(0.11, 0.09, 0.11)
    ...
    fun goToPosition(azusa: Azusa, target: Waypoint) {
        ...
        movementPowers.x *= relTarget.x.absoluteValue / 12.0
        movementPowers.y *= relTarget.y.absoluteValue / 12.0

        val deltaH = getDeltaH(azusa.currPose, target)
        val turnPower = deltaH / 30.0.toRadians
    ...
    fun followPath(azusa: Azusa, start: Waypoint, end: Waypoint) {
        val (normalX, normalY) = circleLineIntersection(azusa.currPose.p, start.p, end.p, end.followDistance)
